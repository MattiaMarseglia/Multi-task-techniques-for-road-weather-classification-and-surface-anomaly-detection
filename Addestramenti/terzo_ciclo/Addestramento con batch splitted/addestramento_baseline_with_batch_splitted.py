...
PARAMETERS_AND_NAME_MODEL = "addestramento_baseline_with_batch_splitted_v2"
...
train_data = pd.read_csv(
   '/mnt/sdc1/mmarseglia/dataset/train_reordered.csv',
...
val_data = pd.read_csv(
   '/mnt/sdc1/mmarseglia/dataset/val_reordered.csv',
    names=["image", "label","class"],dtype={'image':'str','label':'str','class':'str'})
...
base_criterion = nn.CrossEntropyLoss(reduction='sum').to(device)
...
def train_model(model, base_criterion, optimizer, scheduler, early_stopper,num_epochs=25,best_loss=0.0, numTrain=1, acc_best_loss=0.0):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            epoch_loss = running_loss / (dataset_sizes[phase]/(batch_size*8))
