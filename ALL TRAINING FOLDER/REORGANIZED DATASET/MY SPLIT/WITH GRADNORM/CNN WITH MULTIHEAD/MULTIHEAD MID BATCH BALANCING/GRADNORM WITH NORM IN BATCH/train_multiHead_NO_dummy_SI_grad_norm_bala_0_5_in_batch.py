...
PARAMETERS_AND_NAME_MODEL = "train_multiHead_without_dummy_variables_REAL_grad_norm_in_batch_new_split_balanced_0_5"
...
def harmonic_flattening(array, flattening_coeff):
    # Calcola la media degli elementi nell'array
    mean = np.mean(array)
    # Calcola la differenza tra ciascun elemento e la media
    diff_array = array - mean
    # Applica l'appiattimento armonico
    flattened_array = mean + diff_array * flattening_coeff
    return flattened_array

single_classes = [labels for labels in df_train['label']]
# Dizionario per tenere traccia del conteggio
counts_dict = {}

# Scorrere la lista e aggiornare il conteggio nel dizionario
for sublist in single_classes:
    # Converti la sottolista in una tupla per poterla utilizzare come chiave del dizionario
    sublist_tuple = sublist
    
    # Aggiorna il conteggio nel dizionario
    counts_dict[sublist_tuple] = counts_dict.get(sublist_tuple, 0) + 1
# counts,bins = np.histogram(single_classes, np.array(list(class_dictionary.values()))) #how much samples are there for each age 
count_values=np.array(list(counts_dict.values()))
weights = (1/count_values)*1000
weights = harmonic_flattening(weights,0.5)
tmp=list(counts_dict.keys())
samples_weight = np.array([weights[tmp.index(t)] for t in df_train['label']])
samples_weight = torch.from_numpy(samples_weight)
print(weights)


sampler = torch.utils.data.WeightedRandomSampler(samples_weight.type('torch.FloatTensor'), len(df_train), replacement = False)

train_set = torch.utils.data.DataLoader(trainDataSet,sampler=sampler,batch_size=batch_size,num_workers=3)
...
best_loss = 100000
model_ft,best_loss,train_losses,val_losses = trainer.train(criterion=criterion, optimizer=optimizer, scheduler=exp_lr_scheduler, num_epochs=8,best_loss=best_loss,early_stopper=early_stopper,numTrain=1)